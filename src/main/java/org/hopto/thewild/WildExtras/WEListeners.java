...
public class WEListeners implements Listener {
    ...
    public void onBlockPlace(BlockPlaceEvent ev) { // block break event - or is it?
        if (ev.getBlock().getWorld().getName().contains("old")) {
            debugmsg("Stopping block placement by " + ev.getPlayer().getName() + " in old world");
            ev.setCancelled(true);
        } else {
            // For debugging block placement issues
            debugmsg("Attempt to place block type " + ev.getBlockPlaced().getType() + " by " + ev.getPlayer().getName()
                    + " at " + ev.getBlockPlaced().getLocation() + " - cancelled: " + ev.isCancelled());
        }
    }
    ...
    File protecteduserdata = new File(Bukkit.getServer().getPluginManager().getPlugin("WildExtras").getDataFolder(),
    ...
    void onAnyDamage(EntityEvent event) {
        ...
        if (isVisiting(victim)) {
            debugmsg(event + " damage to " + victim.getName() + " blocked due to visit mode");
            cancelDamage(event);
            return;
        }
        ...
        if (attacker != null) {
            ...
            if (attacker instanceof Projectile) {
                ...
                if (projectile.getShooter() instanceof Player) {
                    ...
                    debugmsg(victim.getName() + " hit by projectile from " + attacker.getName());
                } else {
                    debugmsg("Projectile fired by non-player " + attacker);
                }
            }
        } else {
            debugmsg("Unknown damage source/failed to determine attacker for " + event);
            return;
        }
        ...
        debugmsg("Handling damage on " + victim + " named " + victim.getName() + " from " + attacker);
        ...
        if (isProtected(victim)) {
            ...
            attacker.sendMessage("You are still PvP protected as a new player, and thus cannot attack others yet");
            allowDamage = false;
        }

        if (!allowDamage) {
            cancelDamage(event);
            debugmsg("Preventing damage to " + victim.getName() + " by " + attacker.getName());
        }
        return;
    }
    ...
    /*
     * //prevent pvp damage on visits - and on pvp protect because of spam kills -
     * also prevent protected person from pvping.
     * 
     * @EventHandler(priority=EventPriority.HIGH,ignoreCancelled=true) public void
     * onDamage(final EntityEvent e) { // If it's not a player being damaged, we
     * don't care. if (!(event.getEntity() instanceof Player)) { return; }
     * 
     ...
     * }
     * 
     * final Player victim = (Player)event.getEntity(); if () { IAnyDamageEvent
     * event = (IAnyDamageEvent) e; } else { IAnyDamageEvent event =
     * (IAnyDamageEvent) e; } // If the victim is in visit mode, they're immune to
     * all damage, so nerf // it and go no further if so: if (isVisiting(victim)) {
     * debugmsg(event + " damage to " + victim.getName() +
     * " blocked due to visit mode"); event.setCancelled(true); event.setDamage(0);
     * return; }
     * 
     * // Work out who/what the attacker is. How we do this differs depending // on
     * the type of damage - direct attack or projectile. Entity attacker = null;
     * 
     * if (event instanceof EntityDamageByEntityEvent) { attacker =
     * ((EntityDamageByEntityEvent)event).getDamager(); debugmsg("attacker class " +
     * attacker.getClass()); if (((EntityDamageByEntityEvent)event).getDamager()
     * instanceof Arrow) { debugmsg("damager was instanceof Arrow"); Arrow arrow =
     * (Arrow) ((EntityDamageByEntityEvent)event).getDamager(); if
     * (arrow.getShooter() instanceof Player) { attacker = (Player)
     * arrow.getShooter(); debugmsg( victim.getName() + " hit by arrow from " +
     * attacker.getName() ); } else { debugmsg("Arrow fired by non-player " +
     * attacker); } } } else { debugmsg("Unknown damage source " + event); }
     * 
     * if (attacker == null) {
     * debugmsg("Failed to determine attacker, allow damage"); return; }
     * 
     * // Remainder of checks concern player-to-player damage, so if we //
     * determined it's non-player-caused, ignore.
     * 
     * if (!(attacker instanceof Player)) { debugmsg("Damage from non-player " +
     * attacker); return; }
     * 
     * debugmsg("Handling damage on " + victim + " named " + victim.getName() +
     * " from " + attacker);
     * 
     * // If the player is being show with a bow, though, the "damager" will be //
     * an arrow, and we need to find out who fired that arrow: if (event.getEntity()
     * instanceof Arrow) { debugmsg( victim.getName() + " got shot with an arrow" );
     * Arrow arrow = (Arrow) event.getEntity(); if (arrow.getShooter() instanceof
     * Player) { attacker = (Player) arrow.getShooter(); debugmsg( "Shooter was " +
     * attacker.getName() ); } else { debugmsg( "Shooter was not a player though" );
     * } }
     * 
     * // If the victim is a visiting pmod they should be immune to all damage, //
     * or if the attacker is a visiting pmod they shouldn't be able to // damage
     * other players, so if either is the case shortcut here if (isVisiting(victim)
     * || isVisiting(attacker)) {
     * debugmsg("Victim or attacker is visiting, damage blocked");
     * event.setCancelled(true); event.setDamage(0); return; }
     * 
     * // The remaining damage exemptions all only apply to PvP damage, so if // the
     * attacker isn't a player, we don't care: if (!(attacker instanceof Player)) {
     * return; }
     * 
     * // Right, so we're going to nerf the damage - in some cases we want // to
     * explain why - but only try to explain if the killer is a player boolean
     * allowDamage = true; if (isProtected(victim)) {
     * debugmsg("Protected player can't be attacked");
     * attacker.sendMessage(victim.getName() +
     * " is PvP protected. Leave them alone!"); allowDamage = false; } else if
     * (isProtected(attacker)) { debugmsg("Protected player can't attack others");
     * attacker.sendMessage("You are PvP protected. Type /pvpon to disable");
     * allowDamage = false; } else if (isNewbie(victim)) {
     * debugmsg("Newbie protected from attack");
     * attacker.sendMessage(victim.getName() + " is a newbie, leave them alone!");
     * allowDamage = false; } else if (isNewbie(attacker)) {
     * debugmsg("Newbie can't attack others yet"); attacker.sendMessage(
     * "You are still PvP protected as a new player, and thus cannot attack others yet"
     * ); allowDamage = false; }
     * 
     * if (!allowDamage) { event.setCancelled(true); event.setDamage(0); debugmsg(
     * "Preventing damage to " + victim.getName() + " by " + attacker.getName() ); }
     * return; }
     */
    ...
    private boolean isNewbie(final Entity player) {
        ...
        return false;
        /*
         * debugmsg("is_newbie for " + player.getName() + " found play time " +
         * playtime_secs); boolean isNewbie = (playtime_secs < 60 * 60 * 4);
         * newbieCacheResult.put(player.getName(), isNewbie); newbieCacheTimestamp.put(
         * player.getName(), (long) System.currentTimeMillis() / 1000L );
         * debugmsg("Calculated isNewbie result " + isNewbie + " for " +
         * player.getName()); return isNewbie;
         */
    }
    ...
    public void onTargetEvent(EntityTargetEvent e) {
        ...
        final Location spawnLocation = new Location(piggie.getWorld(), 2601, 70, 2559);
    ...
    /*
     * @EventHandler public void onPlayerEat (PlayerItemConsumeEvent e) { if
     * (e.getItem().getType() == Material.CHORUS_FRUIT) { e.setCancelled(true);
     * debugmsg( "Stopped chorus fruit consumption by " + e.getPlayer().getName() );
     * } else { debugmsg( "Non chorus fruit eating detected, that's fine" ); } }
     * 
     */

    // prevent block pickup by moderators visiting. - need to add stop chest /
    // enderchest open. Also need to stop damage.
    @EventHandler
    ...
    public void joinEvent(PlayerJoinEvent event) {
        ...
        if (!playerfile.exists()) {
            ...
            String myMessage = ChatColor.LIGHT_PURPLE + "Please welcome" + ChatColor.WHITE + " " + playername
                    + ChatColor.LIGHT_PURPLE + " to The-Wild!!";
            try {
            ...
            // Welcome them, too
            event.getPlayer()
                    .sendMessage("Welcome aboard!  Please see the FAQ " + "http://the-wild.tk/faq for helpful info!");
        }
    ...
    /*
     * Attempt to apply the appropriate colour to a player's nick, depending on
     * whether they are a newbie or currently protected, falling back to the default
     * based on their GM group
     */
    public void colorNick(Player player) {
        debugmsg("colorNick called for " + player.getName() + "(" + player.getDisplayName() + ")");
    ...
    /*
     * public String getPlayerGroup(Player player) { final AnjoPermissionsHandler
     * handler = groupManager.getWorldsHolder().getWorldPermissions(player); if
     * (handler == null) { debugmsg("Failed to get permissions handler for " +
     * player.getName()); return null; } else { return
     * handler.getGroup(player.getName()); } }
     * 
     * 
     * 
     * 
     ...
     * 
     * private boolean setupGroupManagerAPI() { final PluginManager pluginManager =
     * Bukkit.getServer().getPluginManager(); final Plugin GMplugin =
     * pluginManager.getPlugin("GroupManager"); if (GMplugin != null &&
     * GMplugin.isEnabled()) { groupManager = (GroupManager)GMplugin;
     * debugmsg("Fetched GroupManager plugin"); return true; } else {
     * debugmsg("Failed to obtain GroupManager plugin"); return false; } }
     */
    ...
    public void checkRailClicks(PlayerInteractEvent e) {
        Player player = e.getPlayer();
        if (e.getClickedBlock() != null && (e.getClickedBlock().getBlockData().getMaterial() == Material.RAIL
            ...
            if (e.getItem() != null) {
                ...
                if (railsIgnoreItem(itemType)) {
                    debugmsg(player.getName() + "clicks rail with " + itemType + " - no auto-minecart");
                    return;
                }
            }
            ...
            if ((player.getInventory().getItemInMainHand() != null
                    && railsIgnoreItem(player.getInventory().getItemInMainHand().getType()))
                    || (player.getInventory().getItemInOffHand() != null
                            && railsIgnoreItem(player.getInventory().getItemInOffHand().getType()))) {
                debugmsg(player.getName() + " clicked while holding ignored item" + " - no auto-minecart");
                return;
            }
            ...
            Location cartLocation = new Location(railLocation.getWorld(), railLocation.getBlockX() + 0.5,
                    railLocation.getBlockY() + 0.25, railLocation.getBlockZ() + 0.5);

            // OK, place a minecart on the clicked rail
            Minecart cart = e.getClickedBlock().getLocation().getWorld().spawn(cartLocation, Minecart.class);
    ...
    private boolean railsIgnoreItem(Material itemType) {
        if (itemType == Material.RAIL || itemType == Material.POWERED_RAIL || itemType == Material.DETECTOR_RAIL
                || itemType == Material.MINECART
        /*
         * FIXME: what to do with these? They're deprecated it seems? || itemType ==
         * Material.POWERED_MINECART || itemType == Material.HOPPER_MINECART || itemType
    ...
    public void logEditedBook(PlayerEditBookEvent e) {
        ...
        plugin.getLogger().info("Player " + player.getName() + " is editing a book titled '" + meta.getTitle()
